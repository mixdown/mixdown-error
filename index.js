...
module.exports = BasePlugin.extend({
  ...
  notAuthenticated: function(err, httpContext, headers) {
    var res = httpContext.response;
    if (!res.headersSent) {
      res.writeHead(401, _.extend({
        'Content-Type': 'text/plain'
      }, headers));
    }

    res.end('401 - Not Logged In ' + this.formatError(err));
  },

  notAuthorized: function(err, httpContext, headers) {
    var res = httpContext.response;
    if (!res.headersSent) {
      res.writeHead(403, _.extend({
        'Content-Type': 'text/plain'
      }, headers));
    }

    res.end('403 - Forbidden ' + this.formatError(err));
  },
